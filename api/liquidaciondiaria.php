...
function obtenerTransaccionesPorLiquidar($dbConnection, $idpdv, $fecha)
{
	...
	$statement = "SELECT nrotransaccion, idpdv, sucursal, importe, metodopago, estadocheque, nrocomprobante, estadocobranza, canal,
...
function obtenerMontosLiquidacionesPendientes($dbConnection, $fecha)
{
	...
	$statement = "SELECT idpdv, sum(importe) as montoMoura , sum(importeprimervenc-importe) as montoPDV
			FROM transacciones t
...
function obtenerMontosLiquidacionesPendientesPorMetodoPago($dbConnection, $idpdv, $fecha)
{
	...
	$statement = "SELECT metodopago, count(*) as cantidadtrans, sum(importe) as montoMoura , sum(importeprimervenc-importe) as montoPDV
			FROM transacciones t
			WHERE t.completada = 0 and t.idpdv = ? and ( DATE(t.fechapago) = ?  or DATE(t.fechapagobind) = ? )
...
{
	$statement = "SELECT t.metodopago, sum(d.importeadevolver) montoDevolucion
			FROM devoluciones d
...
{
	$statement = "SELECT nrodevolucion, nrotransaccion, idpdv, fechadevolucion, importeadevolver,
...
function obtenerMontoDevolucionesPendientes($dbConnection, $idpdv)
{
	...
	$statement = "SELECT sum(importeadevolver - importedevuelto) as monto
			FROM devoluciones
			WHERE idpdv  = ? and completada = 0				
        ";
...
function matchearDevolucion($dbConnection, $nroTransaccion, $nroDevolucion)
{
	...
	$statement = "UPDATE devoluciones d		
...
function actualizarTransaccionesLiquidadas($dbConnection, $idpdv, $idLiq, $metodopago, $fecha)
{
	...
	$statement = "UPDATE transacciones t		
...
function actualizarMontoAhorroPDV($dbConnection, $idpdv, $idLiq)
{
	...
	$statement = "UPDATE puntosdeventa p
			JOIN (
...
function confirmarLiquidacion($dbConnection, $idpdv, $fecha)
{
	...
	foreach ($liquidaciones as $liq) {  //1 registro de liquidacion por cada metodo de pago
		...
		$idLiq = insertarLiquidacion(
			$dbConnection,
			$fechaliq,
			$liq['montoMoura'] + $liq['montoPDV'] - $montoDevolucion,
			$liq['cantidadtrans'],
			'LIQUIDADA',
			$idpdv,
			$fechapago,
...
{
	$statement = "SELECT linea0, linea1, linea2 
