...
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;
	...
	try {		
		...
		$mHexa = strtoupper(dechex(ltrim(substr($fechaurl, 2, 2), '0'))); // Convertir a Hexa Mayúscula (07/10/2025 Diego)
		...
		$mExtensionBIND = strtoupper(dechex(ltrim(substr($fechaExtensionBINDDDMMAA, 2, 2), '0'))); // Mes (con 0 a la izquierda removido si existe). Convertir a Hexa Mayúscula (07/10/2025 Diego)
		...
		if (esSabadoDomingoFeriadoDDMMAA($fechaurl) == 0 ){
			$nombreArchivo = 'A065BOTON' . $fechaurl . '.' . $mHexa . $d;
		...
		//$nombreArchivo = 'A065BOTON011025.A01';
		...
		if (esSabadoDomingoFeriadoDDMMAA($fechaurl) == 0 ){
			$nombreArchivo = 'A065DEVBOTON' . $a . (strlen($m) == 2 ? $m : '0' . $m) . $d . '.' . $mHexa . $d;
		}
		else {
			$nombreArchivo = 'A065DEVBOTON' . $a . (strlen($m) == 2 ? $m : '0' . $m) . $d . '.' . $mExtensionBIND . $dExtensionBIND;
		}
		
		//$nombreArchivo = 'A065DEVBOTON251001.A01';
		descargarArchivo($nombreArchivo);
